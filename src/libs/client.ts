...
export class CosmosRestClient extends BaseRestClient<RequestRegistry> {
  ...
  async getGovProposal(id: string) {
  ...
  async getGovProposalTally(id: string) {
    return this.request(this.registry.gov_proposals_tally, { id });
  }
  async getGovProposalVotes(id: string, page?: PageRequest) {
    if(!page) page = new PageRequest()
    page.reverse = true
    const query =`?proposal_status={status}&${page.toQueryString()}`;
    return this.request(this.registry.gov_proposals_votes, { id }, query);
  }
  async getGovProposalVotesVoter(id: string, voter: string, options: { "option": string, "weight": string }[] ) {
