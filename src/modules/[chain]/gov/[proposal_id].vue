...
const upgradeCountdown = computed((): number => {
  const height = Number(proposal.value.messages?.plan?.height || 0);
  if (height > 0) {
  ...
  const now = new Date();
  const end = new Date(proposal.value.messages?.plan?.time || '');
  return end.getTime() - now.getTime();
});
...
const yes_count = computed(() => {
  if (total.value > 0) {
    const value = proposal.value?.final_tally_result?.yes_count || 0;
    return format.percent(Number(value) / total.value);
  }
  return 0;
});

const no_count = computed(() => {
  if (total.value > 0) {
...
const processList = computed(() => {
  return [
    ...
    { name: 'Yes', value: yes_count.value, class: 'bg-success' },
    { name: 'No', value: no_count.value, class: 'bg-error' },
...
<template>
  <div>
    <div class="bg-base-100 px-4 pt-3 pb-4 rounded mb-4 shadow">
      ...
      <div class="">
        <ObjectElement :value="proposal.messages" />
      </div>
    </div>
    <div class="bg-base-100 px-4 pt-3 pb-4 rounded mb-4 shadow">
      <h2 class="card-title mb-1">Summary</h2>
        <p class="h-20 w-full ">
          {{ proposal?.summary }}
        </p>
    </div>
      ...
      <div class="bg-base-100 px-4 pt-3 pb-5 rounded shadow lg:!!col-span-2">
        ...
        <div class="px-1">
          ...
          <div
            class="mt-4"
            v-if="
              proposal?.messages?.['@type']?.endsWith('SoftwareUpgradeProposal')
            "
          >
            <div class="flex items-center">
              ...
              <div class="text-base flex-1 text-main">
                Upgrade Plan:
                <span v-if="Number(proposal.messages?.plan?.height || '0') > 0">
                  (EST)</span
                >
                <span v-else>{{
                  format.toDay(proposal.messages?.plan?.time)
                }}</span>
              </div>
              <div class="text-sm">
                {{ shortTime(proposal.voting_end_time) }}
